...
# (multi-dx + strong compliance effect + overlay slider + calibration + soft uplift + SHAP + Actions + Batch)
...
# ==============================
# Global knobs (with UI overrides later)
# ==============================
CAL_LOGIT_SHIFT_DEFAULT = float(os.getenv("RISK_CAL_SHIFT", "0.40"))  # 全域校正
BLEND_W_DEFAULT = 0.50                                               # Final = (1−w)*Model + w*Overlay
SOFT_UPLIFT = {"floor": 0.65, "add": 0.20, "cap": 0.95}              # 自傷 soft uplift
BORDER_BAND = 7                                                      # 邊帶寬度（score 0–100）

# ==============================
# Policy weights (log-odds). 強化 Medication Compliance（非線性＋交互作用）
# ==============================
POLICY = {
    "per_prev_admission": 0.15,         # 每多 1 次既往住院 ↑ 0.15（上限 5 次）
    "per_point_low_compliance": 0.40,   # (5 - 順從) 每 1 分 ↑ 0.40（線性項，中心在 5）
    "low_compliance_boost": 0.80,       # 順從 ≤3 額外加成（非線性）
    "zero_compliance_extra": 0.60,      # 順從接近 0 再加成
    "sud_low_compliance_interact": 0.50,# SUD × 低順從 交互作用
    "low_compliance_no_followups": 0.25,# 低順從 且 無追蹤
...
def train_demo_model(columns):
    ...
    # Balanced literature-inspired logits（較高基線）
    beta0 = -0.60
    beta = {
    ...
    model.fit(X, y)   # DataFrame 保留 feature_names
    return model
...
# ==============================
# Sidebar (inputs + calibration sliders)
# ==============================
with st.sidebar:
    ...
    with st.expander("Advanced calibration", expanded=False):
        cal_shift = st.slider("Calibration (log-odds, global)", -1.0, 1.0, CAL_LOGIT_SHIFT_DEFAULT, 0.05)
        blend_w  = st.slider("Overlay weight (blend)", 0.0, 1.0, BLEND_W_DEFAULT, 0.05,
                             help="Final = (1−w)*Model + w*Overlay；想看政策效果→拉高 w")
...
set_onehot_by_prefix_multi(X_final, "diagnosis", diagnoses if len(diagnoses)>0 else ["Other/Unknown"])
...
# ==============================
# Predict (align + overlay + calibration + blend + soft uplift)
# ==============================
X_aligned_df, _ = align_df_to_model(X_final, model)
X_np = to_float32_np(X_aligned_df)
p_model = float(model.predict_proba(X_np, validate_features=False)[:, 1][0])

# ---- Policy overlay（logit space）----
lz = _logit(p_model)
...
# 順從度（線性 + 非線性 + 交互）
comp = float(X_final.at[0, "medication_compliance_score"])
lz += POLICY["per_point_low_compliance"] * max(0.0, 5.0 - comp)
has_sud = bool(int(X_final.at[0, "diagnosis_Substance Use Disorder"])) if "diagnosis_Substance Use Disorder" in X_final.columns else False
no_followups = (float(X_final.at[0, "post_discharge_followups"]) == 0)

if comp <= 3:
    lz += POLICY["low_compliance_boost"]
    if has_sud:
        lz += POLICY["sud_low_compliance_interact"]
    if no_followups:
        lz += POLICY["low_compliance_no_followups"]
if comp <= 0.5:
    lz += POLICY["zero_compliance_extra"]
...
# ==============================
# SHAP (prefer XGBoost pred_contribs; fallback TreeExplainer)
# ==============================
with st.expander("SHAP Explanation", expanded=True):
    st.caption("How to read: positive bars push higher risk; negative bars lower it. Only selected one-hot categories are shown.")
    import xgboost as xgb
    try:
        booster = model.get_booster()
    ...
    for dx in diagnoses if diagnoses else ["Other/Unknown"]:
...
# ==============================
# Recommended Actions (richer set)
# ==============================
...
def action_rows(level_label, row: pd.Series):
    acts = []
    # Base by level
    BASE = {
        "High": [
            ("Today","Clinician","Safety plan + crisis hotline.","High risk score"),
            ("Today","Clinic scheduler","Book return within 7 days.","Rapid follow-up reduces dropout"),
            ("Today","Social worker","Enroll in case management.","Increase linkage & adherence"),
        ],
        "Moderate": [
            ("1–2 weeks","Clinic scheduler","Schedule return.","Maintain continuity"),
            ("1–2 weeks","Nurse","Check adherence barriers.","Address practical issues"),
        ],
        "Low": [
            ("2–4 weeks","Clinic scheduler","Routine follow-up.","Standard care"),
            ("2–4 weeks","Nurse","Provide education materials.","Reinforce engagement"),
        ],
    }
    bucket = {
        "High":"High",
    ...
    acts += BASE[bucket[level_label]]

    # Policy-driven personalized
    comp = float(row.get("medication_compliance_score", 0))
    fu = float(row.get("post_discharge_followups", 0))
    if comp <= 3:
        acts.append(("Today","Pharmacist","Medication review & blister pack.","Low compliance ≤3"))
        if has_dx(row, "Substance Use Disorder"):
            acts.append(("Today","Addiction team","Motivational interview; consider MAT.","SUD × low compliance"))
        if fu == 0:
            acts.append(("Today","Care coordinator","Book first follow-up before discharge.","No follow-ups"))

    if flag_yes(row, "has_recent_self_harm") or flag_yes(row, "self_harm_during_admission"):
        acts.append(("Today","Clinician","C-SSRS + update safety plan.","Self-harm flag"))

    if has_dx(row, "Personality Disorder"):
        acts.append(("1–2 weeks","Therapist","Offer DBT/skills group referral.","PD increases early dropout"))

    if has_dx(row, "Schizophrenia"):
        acts.append(("1–2 weeks","Clinician","Assess for LAI candidacy.","Improves adherence"))

    if row.get("family_support_score", 5) < 4:
        acts.append(("1–2 weeks","Social worker","Family meeting / psychoeducation.","Low family support"))

    return acts

rows = action_rows(level, X_final.iloc[0])
# 3 欄呈現
c_timeline, c_owner, c_action = st.columns([1,1,3])
with c_timeline:
    st.markdown("**Timeline**");       [st.write(tl) for tl,_,_, in [(r[0],r[1],r[2]) for r in rows]]
with c_owner:
    st.markdown("**Owner**");          [st.write(ow) for _,ow,_, in [(r[0],r[1],r[2]) for r in rows]]
with c_action:
    st.markdown("**Action**");         [st.write(ac) for _,_,ac, in [(r[0],r[1],r[2]) for r in rows]]
...
if level == "High":
    def make_sop_txt(score: int, label: str, actions: list) -> BytesIO:
        ...
        for (tl, ow, ac) in [(a[0],a[1],a[2]) for a in actions]:
            lines.append(f"- {tl} | {ow} | {ac}")
        buf = BytesIO("\n".join(lines).encode("utf-8")); buf.seek(0); return buf
    st.download_button("⬇️ Export SOP (TXT)", make_sop_txt(score, level, rows),
    ...
    if HAS_DOCX:
        ...
        st.download_button("⬇️ Export SOP (Word)", make_sop_docx(score, level, rows),
...
if uploaded is not None:
    try:
        ...
        # Null-safe numeric getter
        def safe_get_num(col, default=0.0):
        ...
        df["age"] = safe_get_num("Age", 0)
        df["length_of_stay"] = safe_get_num("Length of Stay (days)", 0)
        df["num_previous_admissions"] = safe_get_num("Previous Admissions (1y)", 0)
        df["medication_compliance_score"] = safe_get_num("Medication Compliance Score (0–10)", 5)
        df["family_support_score"] = safe_get_num("Family Support Score (0–10)", 5)
        df["post_discharge_followups"] = safe_get_num("Post-discharge Followups", 0)

        def apply_onehot_prefix_multi(human_col, prefix, options):
            if human_col not in raw.columns:
                return
            ser = raw[human_col].astype(str).fillna("")
            for i, cell in ser.items():
        ...
        if "Diagnoses" in raw.columns:
            ...
            # 若完全沒有診斷欄，標 Other/Unknown 以免空白
            df["diagnosis_Other/Unknown"] = 1
        ...
        # ---- Vectorized policy overlay（含強化順從度規則）----
        ...
        # 順從度（線性 + 非線性 + 交互）
        comp = df["medication_compliance_score"].astype(float).to_numpy()
        lz += POLICY["per_point_low_compliance"] * np.maximum(0.0, 5.0 - comp)

        has_sud = (df.get("diagnosis_Substance Use Disorder", 0).to_numpy() == 1)
        no_followups = (df["post_discharge_followups"].astype(float).to_numpy() == 0)

        low_comp_mask = (comp <= 3)
        zero_like_mask = (comp <= 0.5)

        lz += POLICY["low_compliance_boost"] * low_comp_mask
        lz += POLICY["sud_low_compliance_interact"] * (low_comp_mask & has_sud)
        lz += POLICY["low_compliance_no_followups"] * (low_comp_mask & no_followups)
        lz += POLICY["zero_compliance_extra"] * zero_like_mask

        # 家庭支持、追蹤、社工
        fam = df["family_support_score"].astype(float).to_numpy()
        lz += POLICY["per_point_low_support"] * np.maximum(0.0, 5.0 - fam)
